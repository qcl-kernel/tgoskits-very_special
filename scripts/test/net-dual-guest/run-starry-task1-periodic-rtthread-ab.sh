...
# Measure RT-Thread periodic wake-up latency while StarryOS runs real
# ncnn/YOLO. The two arms differ only in the AxVisor scheduler feature.
...
qemu_config="$repo_root/scripts/test/net-dual-guest/qemu-aarch64-starry-rtthread-task1-periodic-capture.toml"
...
grep -qx 'period_ms = "10"' "$periodic_manifest"
...
cp "$periodic_manifest" "$output_root/rtthread-periodic.manifest"
cat > "$output_root/protocol.txt" <<EOF
experiment=starryos-yolo-rtthread-periodic-scheduler-ab
single_variable=axvisor-scheduler-feature
arms=rr,fp-rr
repeats_per_arm=$repeats
run_order=rr,fp-rr,repeated
topology=starry-vcpu0->pcpu1,rtthread-vcpu0->pcpu1
starry_host_priority=89
rtthread_host_priority=90
rtthread_period_ms=10
rtthread_samples=300
endpoint_mode=model-only
...
cmd (sh /usr/bin/t2n1-run.sh model-only) &
...
printf 'PASS: StarryOS Task 1 RT-Thread periodic A/B retained in %s\n' "$output_root"
