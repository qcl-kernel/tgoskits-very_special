...
#[cfg(feature = "arceos")]
fn is_would_block(error: &ax_std::StdError) -> bool {
    match *error {
        ax_std::StdError::Api(error) => {
            ax_std::io::Error::from(error) == ax_std::io::ErrorKind::WouldBlock
        }
        ax_std::StdError::Io(error) => error == ax_std::io::ErrorKind::WouldBlock,
        _ => false,
    }
}
