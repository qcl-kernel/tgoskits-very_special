...
为把“模型输出数字”转换成可以直接观察的交通行为，本项目使用
**CARLA 0.9.16** 构建自动驾驶仿真环境。CARLA 提供道路、车辆、行人、天气、
车载相机和碰撞传感器；同一个场景分别运行固定基线与 YOLO 视觉闭环，A/B
之间不更换道路和障碍物故事。

- **A（固定基线）**：车辆保持固定控制输出，不根据前方车辆或行人避让；
- **B（YOLO 闭环）**：检测车辆/行人，执行减速、Stop、2 s Confirm、Reset、
  Resume，并在 Reset 后继续正常行驶至少 6 s。

| 场景 | 交通故事 | 固定基线 | YOLO 闭环 |
| --- | --- | --- | --- |
| 1. 晴天直路静止汽车 | 本车接近同车道静止汽车 | 碰撞 | 停车、确认、Reset、恢复，零碰撞 |
| 2. 斑马线行人横穿 | 行人横穿本车行驶区域 | 碰撞 | 最小实际净距 `2.223 m`，零碰撞 |
| 3. 前车切入并制动 | 动态车辆切入后成为前向障碍 | 碰撞 | 跟踪并停车，危险解除后恢复，零碰撞 |
| 4. 货车遮挡行人 | 货车遮挡条件下出现危险行人 | 碰撞 | 最小实际净距 `14.104 m`，零碰撞 |
| 5. 多车多人复杂路口 | 多辆车、多名行人与背景目标同时出现 | 碰撞 | 选择相关危险目标，最小行人净距 `2.217 m`，零碰撞 |

汇总结果为：固定基线 `5/5` 发生碰撞；YOLO 闭环 `5/5` 碰撞数为 0，
并完整经过 `RUN -> STOP -> CONFIRM -> RESET -> RESUME -> RUN`。场景 2、4、5
以车身外沿到行人外沿的几何净距核验，而不是只依赖碰撞传感器；三者均达到
`2 m` 验收线。每个 YOLO 场景在 Reset 后继续行驶约 `6.03 s`，用于明确展示
车辆已解除停车锁存并恢复正常运行。

CARLA 闭环控制由主机 ONNX Runtime 执行，视频中按场景显示的 RK3588 NPU、
T2N1 CONTROL/STATUS 和 Reset 回执来自五场景代表帧的真实板卡运行。二者组成
“仿真交通行为 + 实板推理/通信测量”的证据链，但不宣称 CARLA 相机流逐帧实时
传输到开发板。正式物理板仍严格使用 StarryOS `[0x200, 0x100]`、Zephyr
`[0x100]`、NPU 仅归 StarryOS 的冻结拓扑。

视频按一个 B站稿件的 10 个分 P 组织，顺序为每个场景的固定基线、YOLO 闭环：

**B站多 P 播放链接：** https://www.bilibili.com/video/BV11XhF6REHU

仿真说明、逐帧 JSON、实板日志和哈希索引位于
[`results/task3/carla-five/`](../results/task3/carla-five/)。
