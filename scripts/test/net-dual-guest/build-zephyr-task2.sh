...
periodic_dump_chunk_rows="${TASK1_ZEPHYR_DUMP_CHUNK_ROWS:-256}"
...
fdt_path="${TASK2_ZEPHYR_VIRTIO_FDT_PATH:-$fdt_path}"
host_hwirq="${TASK2_ZEPHYR_VIRTIO_HOST_HWIRQ:-$host_hwirq}"
guest_irq="${TASK2_ZEPHYR_VIRTIO_GUEST_IRQ:-$guest_irq}"
if [[ "$fdt_path" != /* ]]; then
    printf 'error: TASK2_ZEPHYR_VIRTIO_FDT_PATH must be an absolute FDT path\n' >&2
    exit 1
fi
if [[ ! "$host_hwirq" =~ ^[0-9]+$ || ! "$guest_irq" =~ ^[0-9]+$ ]]; then
    printf 'error: TASK2_ZEPHYR_VIRTIO_HOST_HWIRQ and TASK2_ZEPHYR_VIRTIO_GUEST_IRQ must be non-negative integers\n' >&2
    exit 1
fi
case "$fault_mode" in
    none)            fault_define=0 ;;
...
if [[ ! "$periodic_dump_chunk_rows" =~ ^[1-9][0-9]*$ ]]; then
...
if [[ "$zephyr_source_revision" != "$zephyr_revision" ]]; then
...
ZEPHYR_BASE="$zephyr_base" cmake --fresh -S "$source_dir" -B "$build_dir" -G Ninja \
    ...
    -DRT_DUMP_CHUNK_ROWS="$periodic_dump_chunk_rows" \
    "${extra_conf_args[@]}" \
...
zephyr_version="$zephyr_source_revision"
manifest="$out_dir/manifest.toml"
{
    ...
    printf 'periodic_dump_chunk_rows = "%s"\n' "$periodic_dump_chunk_rows"
    printf 'runtime_trace = "quiet-during-periodic-sampling"\n'
